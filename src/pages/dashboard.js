...
const createContentTableForScore = (
  questionGroup = "Easy",
  answerScoreArrs = []
) => {
  return (
    <div>
      <table class="table">
        <thead>{generateHeaderRowQuestionair(questionGroup)}</thead>
      ...
      <table className="mx-auto max-w-4xl w-full whitespace-nowrap rounded-box bg-white divide-y divide-gray-300 overflow-hidden">
        <thead class="bg-gray-900">
          <tr class="text-white text-left">
            <th className="text-sm uppercase px-6 py-4 text-center">
              Easy (1 point)
            </th>
            <th className="text-sm uppercase px-6 py-4 text-center">
              Medium (3 point)
        ...
        <tbody>
          <tr>
            ...
            <td className="text-sm px-6 py-4">
              {createContentTableForScore("Medium")}
            </td>
            <td className="text-sm px-6 py-4">
              {createContentTableForScore("Hard")}
            </td>
          </tr>
        </tbody>
      </table>
    ...
    return (
      <>
        <tbody className="group animate-in fade-in border-opacity-0 bg-gray-800 hover:bg-gray-700">
          <tr
            key={index}
            clasName="group-hover:bg-700 border-opacity-0 bg-white bg-gray-800 border-0"
          >
            <th
              scope="row"
              class="text-xl px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white max-w-[10px]"
            >
              {index + 1}
            </th>
          ...
          <tr className="animate-in fade-in  dark:bg-gray-900">
            <td colSpan={12}>
              <div
                tabindex={index}
                className="collapse focus:outline-none"
                id={"hover-input-" + index}
              >
      ...
      <div className="w-full mx-auto px-24">
        <div class="relative overflow-x-auto h-[100vh] shadow-md sm:rounded-lg ">
          <table class="w-full text-left text-gray-500 dark:text-gray-400 overscroll-y-auto">
            <thead
              className="text-xs sticky top-0 py-3 text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400"
